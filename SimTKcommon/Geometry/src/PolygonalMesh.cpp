...
createBrickMesh(const Vec3& halfDims, int resolution) {
    ...
    for (int i=0; i<3; ++i)
        nv[i] = 1 + std::max((int)(dims[i].getValue()/edgeLengthTarget.getValue() + 0.49), 1);
                   ...
                   int resolution) 
{
    ...
    Real chordLen = 2*radius*sin(angle/2);
    int rezRadial = (int)(radius.getValue()/chordLen.getValue()+0.5);
    Real edgeLenRad = radius/rezRadial;

    int rezAlong  = 1 + std::max((int)(halfLength.getValue()/edgeLenRad.getValue() + 0.5), 1);
    ...
    for (int k=0; k < rezAlong; ++k) {
        ...
        for (int a=0; a < rezAround; ++a) {
            Real x = edgeLenRad*sin(a*angle);
            Real y = edgeLenRad*cos(a*angle);
